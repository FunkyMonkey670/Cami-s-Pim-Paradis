...
export async function createLogicBundle(project) {
  ...
  if (project.header) {
    lines.push(
      "# ##############################################################################",
      "# CAMI'S PIM PARADIS - GENERATED MAP LOGIC",
      "# DO NOT EDIT THIS BUNDLED FILE; EDIT THE REPOSITORY SOURCE FILES INSTEAD",
      "# BUILD MANIFEST: server/" + project.manifestDisplayPath,
      "# ORDERED BUILD UNITS: " + sources.length,
      "# ERROR LOOKUP: tools\\cl-bundler.cmd map <PimParadisFull runtime line>",
      "# ##############################################################################",
      "",
    );
  }
  ...
  for (let index = 0; index < sources.length; index += 1) {
    ...
    let markerLine = null;
    let markerStartLine = null;
    let markerEndLine = null;
    if (project.sourceMarkers) {
      if (lines.length > 0 && lines.at(-1) !== "") lines.push("");
      markerStartLine = lines.length + 1;
      const unitNumber = String(index + 1).padStart(3, "0");
      const unitTotal = String(sources.length).padStart(3, "0");
      const unitKind = source.displayPath.startsWith("assets/")
        ? "GENERATED ASSET"
        : "MODULE";
      const unitDeclarations = findDeclarations(source.content, source.displayPath)
      ...
      lines.push(
        ...
        "# BUILD UNIT " +
          ...
          unitKind +
          " | " +
          path.basename(source.displayPath),
        "# REPOSITORY PATH: server/" + source.displayPath,
        "# DECLARATIONS: " + (unitDeclarations || "none detected"),
        "# SOURCE LINES: 1-" + sourceLines.length,
        "# ##############################################################################",
      );
      markerEndLine = lines.length;
      markerLine = markerStartLine;
    }
...
export async function createMapBundle(project, logicBundle) {
  ...
  const embeddedSources = logicBundle.lineMap.sources.map((source) => ({
    ...
    markerStartLine:
      source.markerStartLine === null ? null : source.markerStartLine + logicOffset,
    markerEndLine:
      source.markerEndLine === null ? null : source.markerEndLine + logicOffset,
  }));
...
export function resolveGeneratedLine(lineMap, generatedLine) {
  ...
  for (const source of lineMap.sources ?? []) {
    ...
    if (
      ...
      source.markerStartLine !== undefined &&
      generatedLine >= source.markerStartLine &&
      generatedLine <= source.markerEndLine
    ) {
